...
class TrackpadEvents {
    ...
    /// was touching the trackpad. That is upstream's #5911 (ported from 1a85669b): unusable cursor over
    /// the Dock and the menu bar, no CPU spike anywhere. Listening costs the WindowServer nothing: it
    /// hands us a copy and moves on.
    ...
    /// return true (absorb happens when the switcher is in use, or on the single event that fires the
    /// trigger), so being absent from the stream changes no behaviour.
    ...
    static func disableForSafety() {
        guard detectTap != nil else { return }
        toggle(false)
    }
    ...
    static func reset() {
        ...
        // until the next trackpad touch re-evaluated it — harmless, but it is exactly the state the tap
        // split exists to avoid, and a session that ends with no finger down is the common case (focus on
        // release).
    ...
    private static func observe_() {
        ...
        // (upstream measured on macOS 26.5 with two taps at the same point: the one created SECOND runs
        // FIRST, and swapping the `CFRunLoopAddSource` order changed nothing). The absorbing tap consults
        // a verdict the detecting tap writes, so detection has to run first — i.e. be created LAST.
        // Created the other way round, `absorbGestures` is always one event stale, which would leak
        // precisely the event that fires the trigger.
        ...
        detectTap = CGEvent.tapCreate(
            ...
            place: .headInsertEventTap,
            options: .listenOnly, // see `detectTap`: an active tap here gates the cursor on us
            eventsOfInterest: NSEvent.EventTypeMask.gesture.rawValue,
    ...
    private static func touchEventHandler(_ cgEvent: CGEvent) -> Bool {
        ...
        // never puts an active tap in the HID stream.
        setAbsorbTapEnabled(App.appIsBeingUsed || fingersDown >= requiredFingers)
