...
| Q-08 | Importierte oder migrierte Settings aktivieren nie automatisch Input- oder AX-Module. **Benannte Ausnahmen, entschieden 2026-08-14**: die Creator's Settings duerfen `hyperKeyEnabled` sowie die beiden Drag-Modifier (`windowDragModifier` = Command+Control, `windowResizeModifier` = Command+Option) setzen. **Erweitert am 2026-09-17**: auch `flickRingEnabled` (FlickRing auf der mittleren Maustaste). `leaderEnabled` ist keine Ausnahme und bleibt technisch ausgeschlossen. Sie sind kein Import, sondern eine ausdrueckliche Uebernahme hinter einer bestaetigten Zusammenfassung. Bedingung: die Zusammenfassung benennt jedes scharfgeschaltete Modul in einem eigenen Satz (Caps-Lock-Umbelegung; Move/Resize samt Drag-on-Gesture-Uebernahme durch Command+Control; Uebernahme der mittleren Maustaste durch FlickRing). Fuer Import, Migration und jeden anderen Pfad gilt die Regel unveraendert | Verhindert unbeabsichtigtes Scharfschalten nach Settings-Uebernahme |
...
- **Umgesetzt und am 2026-08-14 nachgemessen.** Die Suche nach `Appearance` findet den Bereich weiterhin — aber **zufaellig**: der Treffer ist die Teilzeichenkette `appear` im Beschreibungstext *"where it appears"*, nicht der frühere Titel. Aendert jemand diese Beschreibung, ist die Auffindbarkeit unbemerkt weg. Wer sie garantieren will, braucht einen ausdruecklichen Suchbegriff-Alias; ohne konkreten Bedarf ist das nicht gebaut worden.
...
**Creator's Settings: Drag-Modifier einschalten — entschieden am 2026-08-14, umgesetzt.** Der Nutzer hat entschieden, dass die Creator's Settings auch Move (`Command+Control`) und Resize (`Command+Option`) scharfschalten. Q-08 traegt die erweiterte Ausnahme; die Zusammenfassung benennt beide Module und die Drag-on-Gesture-Uebernahme in einem eigenen Satz. Der technische Ausschluss bleibt nur noch fuer `nextWindowGesture` bestehen. Das Scharfschalten selbst laeuft ueber den normalen Launch-Pfad nach dem Restart, der die Besitzuebernahme des Drag-on-Gesture-Schalters verifiziert und `Command+Control` bei Misserfolg ablehnt statt halb zu aktivieren.

**Creator's Settings: HyperKey einschalten — entschieden am 2026-08-14, umgesetzt.** Der Nutzer hat sich dafuer entschieden. Q-08 wurde geaendert und traegt die Ausnahme jetzt ausdruecklich; `hyperKeyEnabled` ist aus `excludedInputModuleKeys` entfernt und steht in `values`. Die Zusammenfassung nennt die Caps-Lock-Umbelegung in einem eigenen Satz, wie es die Bedingung unten verlangt. Der technische Ausschluss bleibt fuer `windowDragModifier` und `nextWindowGesture` bestehen. Der urspruengliche Entscheidungstext steht unveraendert darunter.

**Ursprungliche Vorlage — Entscheidung noetig, keine Aufgabe.** Der Wunsch steht gegen **Q-08**: importierte oder uebernommene Settings aktivieren nie automatisch Input- oder AX-Module. `hyperKeyEnabled` steht deshalb in `CreatorSettings.excludedInputModuleKeys`, und der Ausschluss wird beim Anwenden technisch erzwungen, damit ein spaeterer Eintrag in `values` nicht stillschweigend einen Tap scharf schaltet.
