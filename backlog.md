...
- Presets statt Defaults: das sinnvolle Set wird als benannte Zuweisung per Klick angeboten und ebenso wieder entfernt. `macOS Spaces` belegt `Control` plus 1 bis 9 sowie `Control+0` fuer den Toggle. Ein vom Nutzer selbst gesetzter Shortcut wird nie ueberschrieben, sondern gemeldet und ausgelassen.
