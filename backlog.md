...
| S-12 | Zeigerwerte ueber die Anmeldung hinaus | **Beantwortet 2026-08-13: nein.** Vor dem Reboot stand die Maus auf 0.875 ohne jede Praeferenz, danach auf 0.6875, dem Vorgabewert. Der Mechanismus ist gemessen und nicht nur erschlossen: die Systemeinstellungen schreiben beim Reglerzug **beides**, `com.apple.mouse.scaling` *und* den HID-Wert, waehrend AltTab+ nur den HID-Wert schreibt — waehrend AltTab+ 3.0 hielt, existierte die Praeferenz gar nicht. Ohne Praeferenz gibt es bei der Anmeldung nichts anzuwenden. Vorbehalt: beim Reboot hing eine andere Maus am Rechner. Folge: die vom Umbau selbst notierte Konsequenz ist faellig — beim Start neu anwenden, nicht die Praeferenz zusaetzlich schreiben |
