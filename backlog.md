...
| Q-08 | Importierte oder migrierte Settings aktivieren nie automatisch Input- oder AX-Module. **Benannte Ausnahmen, entschieden 2026-08-14**: die Creator's Settings duerfen `hyperKeyEnabled` sowie die beiden Drag-Modifier (`windowDragModifier` = Command+Control, `windowResizeModifier` = Command+Option) setzen. Sie sind kein Import, sondern eine ausdrueckliche Uebernahme hinter einer bestaetigten Zusammenfassung. Bedingung: die Zusammenfassung benennt jedes scharfgeschaltete Modul in einem eigenen Satz (Caps-Lock-Umbelegung; Move/Resize samt Drag-on-Gesture-Uebernahme durch Command+Control). Fuer Import, Migration und jeden anderen Pfad gilt die Regel unveraendert | Verhindert unbeabsichtigtes Scharfschalten nach Settings-Uebernahme |
