...
| 8 | **failed, then fixed, not measured again** — `applicationWillTerminate` releases both categories since `14e8321c` (PR #21, 2026-08-14), which measured `quit while managed -> 0.6875 restored`. Force quit and SIGTERM still skip that path (see `src/main.swift`). The row below records the state before the fix. **Run this step again.** Nothing was restored at quit, not even through the app's own *Quit AltTab+* button. The next launch restores it via `recoverAfterUncleanExit`. The exposure is therefore temporal, not permanent: whoever uninstalls AltTab+ or disables its autostart keeps the foreign value |
| 9 | pass |
| 10 | pass |
