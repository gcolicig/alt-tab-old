...
| 8 | **pass after the fix, measured 2026-09-16** — trackpad set to *Custom* at speed 2.5 in AltTab+, record `{"baseline":0.6875,"state":"managed","lastWritten":2.5}`, effective value 2.5. Quit through the regular quit path (Apple Event, which runs `applicationWillTerminate`): effective value 0.6875 at once and still 5 s later, record `{"state":"unmanaged"}`, the mouse untouched at 0.6875. The fix is `14e8321c` (PR #21, 2026-08-14). Force quit and SIGTERM still skip that path (see `src/main.swift`); the next launch recovers through `recoverAfterUncleanExit`. Before the fix (2026-08-13): nothing was restored at quit, not even through the app's own *Quit AltTab+* button. The next launch restores it via `recoverAfterUncleanExit`. The exposure is therefore temporal, not permanent: whoever uninstalls AltTab+ or disables its autostart keeps the foreign value |
| 9 | pass |
| 10 | pass |
